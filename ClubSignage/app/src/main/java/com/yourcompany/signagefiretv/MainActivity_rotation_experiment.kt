...
class MainActivity : AppCompatActivity() {
    ...
    private fun parsePlaylistResponse(responseText: String): JSONArray? {
        ...
        return if (trimmed.startsWith("[")) {
            ...
            try {
                ...
                applyVideoResizeMode(lastVideoWidth, lastVideoHeight)
    ...
    private fun applyVideoResizeMode(videoWidth: Int, videoHeight: Int) {
    ...
    private fun loadImageInto(target: ImageView, url: String, onReady: (() -> Unit)? = null) {
    ...
    private fun applyImageRotation(item: JSONObject) {
        val degrees = item.optInt("rotation_degrees", 0)
        currentRotationDegrees = degrees
        imageView.rotation = 0f
        imageViewAlt.rotation = 0f
        overlayLogo.rotation = degrees.toFloat()
        imageView.post { applyImageMatrix(imageView) }
        imageViewAlt.post { applyImageMatrix(imageViewAlt) }
        overlayLogo.post { updateOverlayPosition() }
    }

    private fun applyVideoRotation(item: JSONObject) {
        val degrees = item.optInt("rotation_degrees", 0)
        currentRotationDegrees = degrees
        val rotation = degrees.toFloat()
        videoView.rotation = rotation
        overlayLogo.rotation = rotation
        videoView.post { updateVideoRotationScale() }
        overlayLogo.post { updateOverlayPosition() }
    }

    private fun applyImageMatrix(target: ImageView) {
        val drawable = target.drawable ?: return
        val viewW = target.width.toFloat()
        val viewH = target.height.toFloat()
        if (viewW <= 0f || viewH <= 0f) {
            target.post { applyImageMatrix(target) }
            return
        }
        ...
        val rotationDegrees = (currentRotationDegrees % 360 + 360) % 360
        val rotated = rotationDegrees == 90 || rotationDegrees == 270
        val effectiveW = if (rotated) imgH else imgW
        val effectiveH = if (rotated) imgW else imgH
        val scale = minOf(viewW / effectiveW, viewH / effectiveH)

        val matrix = Matrix()
        // Move image center to origin, rotate, scale, then move to view center.
    ...
    private fun updateVideoRotationScale() {
        val rotationDegrees = (currentRotationDegrees % 360 + 360) % 360
        val rotated = rotationDegrees == 90 || rotationDegrees == 270
        if (!rotated) {
            videoView.scaleX = 1f
        ...
        val videoW = lastVideoWidth.toFloat()
        val videoH = lastVideoHeight.toFloat()
        if (videoW <= 0f || videoH <= 0f) return
        val rotatedW = videoH
        val rotatedH = videoW
        val scale = minOf(parentW / rotatedW, parentH / rotatedH)
    ...
    private fun updateOverlayAppearance() {
        ...
        overlayLogo.rotation = currentRotationDegrees.toFloat()
        ...
        lp.setMargins(0, 0, 0, 0)
                when (resolveOverlayPositionForRotation()) {
            "top-left" -> {
    ...
    private fun updateOverlayPosition() {
        ...
        val rotated = (currentRotationDegrees % 180) != 0
        val rotatedW = if (rotated) h else w
        val rotatedH = if (rotated) w else h
        val pos = resolveOverlayPositionForRotation()
        val anchorX = if (pos.contains("left")) 0f else (rootW - rotatedW).toFloat()
        val anchorY = if (pos.contains("top")) 0f else (rootH - rotatedH).toFloat()
        val x = anchorX + (rotatedW - w) / 2f
        val y = anchorY + (rotatedH - h) / 2f
        overlayLogo.x = x
        overlayLogo.y = y
    }

    private fun resolveOverlayPositionForRotation(): String {
        val positions = listOf("top-left", "top-right", "bottom-right", "bottom-left")
        val currentIndex = positions.indexOf(overlayPosition).takeIf { it >= 0 } ?: 1
        val steps = (((currentRotationDegrees % 360) + 360) % 360) / 90
        val mappedIndex = (currentIndex + steps) % positions.size
        return positions[mappedIndex]
    }
